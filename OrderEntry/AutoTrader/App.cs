...
namespace AutoTrader
{
    public class App(ILogger<App> logger, IDatabaseService databaseService, IPolygonApiService polygonApiService, ICoveredCallStrategy coveredCallStrategy)
    {
        ...
        public async Task Run()
        {
            ...
            await polygonApiService.FillEndOfDayData(expirationThreeMonthsAgo.DayNumber - today.DayNumber, Ticker);
            ...
            logger.LogInformation("The estimated price for next month is {estimatedPrice} based on price going from {firstPrice} on {firstDate} to {lastPrice} on {lastDate}", estimatedPriceNextMonth, firstHistoricalPrice.Close, firstHistoricalPrice.From, lastHistoricalPrice.Close, lastHistoricalPrice.From);
            ...
            foreach (var contract in contracts.Where(c => Convert.ToDouble(c.StrikePrice) <= strikeLimit).TakeLast(10))
            {
                ...
                logger.LogInformation("{premium} required for {strikePrice} with profit {profit}", minPremium, contract.StrikePrice, estimatedPriceNextMonth - Convert.ToDouble(contract.StrikePrice) - minPremium);
            }
        }
    }
}
